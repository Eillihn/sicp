...
;; easily change the currency used by the program, so that we could compute the number 
;; of ways to change a British pound, for example. 
;; As the program is written, the knowledge of the currency is distributed partly into the
;; procedure first-denomination and partly into the procedure count-change (which knows 
;; that there are five kinds of U.S. coins). It would be nicer to be able to supply a 
...
;; Answer:
;; The order of the list coins-values does not affect the answer produced by cc, bacause 
;; the procedure computes all possible combinations.
;; But it may affect the execution time.
